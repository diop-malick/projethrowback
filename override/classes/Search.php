...
* Changes :
* Add Preview on ajax search suggestion
*/
...
class Search extends SearchCore
{
        ...
        $order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null)
    {
        ...
        if ($ajax) {
            ...
            $ajaxresult = $db->executeS($sql, true, false);
            if(!$ajaxresult)
                return false;
            foreach($ajaxresult as $akey => $aval) {
                $cv = Image::getCover($aval['id_product']);
                $ajaxresult[$akey]['img'] = $context->link->getImageLink($aval['prewrite'], $cv['id_image'], 'cart_default');
            }
            return $ajaxresult;
        }
